...
CreateObstacle PROC	 
	...
	random:
		...
		call RandomRange
		mov randVal,eax
	;random generate mushroom on two of three floors
	START:
		.IF randVal==0 
		...
		.IF randVal==2
			...
			call Createmushroom2
		.ENDIF
		;wait a moment to implement the mushroom move
		INVOKE Sleep, time	
		;mushroom posotion X COORD-1
		...
		;check if the dinosaur hit the mushroom
		.IF mushroomPosition1.x<=34
			...
			.IF randVal==2
				...
				.ENDIF
			.ENDIF
			;if keyboard is pressed, jump to L3 since zero flag = 0
			call ReadKey
			jnz L3
